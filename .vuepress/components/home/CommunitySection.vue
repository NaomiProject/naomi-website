...
<div class="community">
  ...
  <div class="join">
    ...
    <a target="_blank" href="https://community.projectnaomi.com" class="join-us-button slide-slow">Join Us ➜</a>
  </div>
</div>
</template>
...
export default {
  ...
  mounted () {
    ...
    let url = 'https://community.projectnaomi.com/about.json'
    fetch(url).then((resp) => {
      resp.json().then((json) => {
        if (json.about.stats) {
      ...
      console.log('Failed fetching real-time stats from community.projectnaomi.com - check above for CORS error messages')
    })
  }
}
</script>
